...
# --- SISTEMA DE USUÁRIOS (persistente em CSV) ---
ARQUIVO_USUARIOS = "db_usuarios.csv"
...
def init_usuarios():
    """Cria o CSV de usuários com os padrões se não existir"""
    if not os.path.exists(ARQUIVO_USUARIOS):
        usuarios_padrao = [
            {"usuario": "admin", "senha_hash": hashlib.sha256("gomap2025".encode()).hexdigest(), "nome": "Administrador", "perfil": "admin", "ativo": True},
            {"usuario": "andre", "senha_hash": hashlib.sha256("andre123".encode()).hexdigest(), "nome": "André Lyon", "perfil": "admin", "ativo": True},
            {"usuario": "operador", "senha_hash": hashlib.sha256("op2025".encode()).hexdigest(), "nome": "Operador GOMAP", "perfil": "lancamentos", "ativo": True},
        ]
        pd.DataFrame(usuarios_padrao).to_csv(ARQUIVO_USUARIOS, index=False)
...
def verificar_login(usuario, senha):
    ...
    if len(row) > 0:
        if row.iloc[0]["senha_hash"] == hashlib.sha256(senha.encode()).hexdigest():
            return True, {"nome": row.iloc[0]["nome"], "perfil": row.iloc[0]["perfil"]}
    return False, None
...
# --- 2. BANCO DE DADOS ---
FILES = {
    "mov": "db_movimentacoes.csv",
    "prod": "db_produtos.csv",
    "obras": "db_obras.csv",
    "cat": "db_categorias.csv",
    "unid": "db_unidades.csv",
    "loc": "db_locacoes.csv",
    "equip": "db_equipamentos.csv",
    "forn": "db_fornecedores.csv"
}
...
def init_db():
    schemas = {
        "mov": ["Data", "Tipo", "Material", "Categoria", "Unidade", "Origem", "Destino", "Qtd", "Valor_Unit", "Valor_Total", "Fornecedor", "Data_NF", "Num_NF", "Observacao", "Usuario"],
        "prod": ["Material", "Categoria", "Unidade"],
        "obras": ["Nome_Obra"],
        "cat": ["Nome"],
        "unid": ["Nome"],
        "loc": ["Descricao", "Qtd", "Valor", "Fornecedor", "Contrato", "Data_Inicio", "Data_Devolucao", "Obra", "Periodo_Inicio", "Periodo_Final", "Venc_Boleto", "Valor_Periodo", "Observacao", "Status", "Usuario"],
        "equip": ["Nome_Equipamento"],
        "forn": ["Nome_Fornecedor", "CNPJ", "Insc_Estadual", "Telefone", "Celular", "Email", "Contato", "Endereco", "Bairro", "Cidade", "Estado", "CEP", "Observacao", "Data_Cadastro"],
    }
    for n, p in FILES.items():
        if not os.path.exists(p):
            if n == "unid":
                pd.DataFrame({"Nome": _UNID_DEFAULTS}).to_csv(p, index=False)
            else:
                pd.DataFrame(columns=schemas[n]).to_csv(p, index=False)

init_db()

def _sync_unidades():
    """Adiciona uma única vez por sessão as unidades de produtos ainda não cadastradas."""
    if st.session_state.get("_unid_synced"):
        return
    st.session_state["_unid_synced"] = True
    df_unid = pd.read_csv(FILES["unid"])
    existing_upper = set(df_unid["Nome"].dropna().astype(str).str.upper().tolist())
    to_add = []
    if os.path.exists(FILES["prod"]):
        df_prod = pd.read_csv(FILES["prod"])
        if "Unidade" in df_prod.columns:
            for u in df_prod["Unidade"].dropna().unique():
                su = str(u).strip()
                if su and su.upper() not in ("NAN", "NONE", "") and su.upper() not in existing_upper:
                    to_add.append(su)
                    existing_upper.add(su.upper())
    if to_add:
        new_rows = pd.DataFrame([{"Nome": u} for u in to_add])
        df_unid = pd.concat([df_unid, new_rows], ignore_index=True)
        df_unid.to_csv(FILES["unid"], index=False)

def load(k):
    return pd.read_csv(FILES[k])
